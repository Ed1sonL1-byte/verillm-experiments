"""
Experiment 4: Quantized Model Inference with Heterogeneous Hardware Verification

Goal: Test verification mechanism when inference uses quantized model
      and verification uses full-precision model on DIFFERENT hardware platforms.

Setup:
- Inference: Quantized model (INT4/AWQ) on Device A (e.g., NVIDIA GPU)
- Verification: Full-precision model (FP16) on Device B (e.g., Mac M-series)
- Different hardware platforms (cross-vendor)
...
- Acceptance rate (expected: lower than Exp2 and Exp3, but still >70%)

Key Research Question:
Does the combination of quantization AND hardware heterogeneity
...
class Exp4QuantizedInferenceHeterogeneous(BaseExperiment):
    def __init__(self, model_name: str, inference_device: str, verification_device: str, num_verifiers: int = 3):
        ...
        """
        super().__init__(
            exp_name=f"exp4_quantized_inference_heterogeneous_{model_name}_{inference_device}_to_{verification_device}",
            output_dir=str(project_root / "data/raw/exp4")
        )
    ...
    def run_single_trial(self, prompt: str, trial_id: int) -> dict:
        ...
        for ver_id, ver_result in enumerate(verification_results):
            ...
            # More relaxed threshold for quantized + heterogeneous (70% instead of 90%)
        ...
        return {
            'experiment': 'exp4_quantized_inference_heterogeneous',
            'trial_id': trial_id,
            ...
            'summary': {
                ...
                'final_verdict': 'PASS' if avg_accept_rate > 0.70 else 'FAIL'
            }
        }
    ...
    def run(self):
        ...
        self.logger.info("=" * 80)
        self.logger.info(f"EXPERIMENT 4: Quantized Inference + Heterogeneous Hardware Verification")
...
def main():
    ...
    # Scenario 1: NVIDIA → Mac (Quantized on NVIDIA, FP16 on Mac)
    # INFERENCE_DEVICE = "cuda:0"
    # VERIFICATION_DEVICE = "mps"

    # Scenario 2: Mac → NVIDIA (Quantized on Mac, FP16 on NVIDIA)
    ...
    print("=" * 80)
    print(f"Experiment 4 Configuration:")
    ...
    exp = Exp4QuantizedInferenceHeterogeneous(
